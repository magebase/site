class StripeInvoiceService
  ...
  def add_managed_services(invoice_id)
    # Get recommendations from pricing service (stored in ai_pricing_json)
    pricing_recommendations = @quote_request.ai_pricing_json&.dig("stripe_product_recommendations") || []

    if pricing_recommendations.empty?
      # Fallback to legacy method if no recommendations stored
      add_legacy_managed_services(invoice_id)
      return
    end

    pricing_recommendations.each do |rec|
      next unless rec["recommended"]

      # Find the actual Stripe product
      stripe_product = find_stripe_product_by_id(rec["product_id"])
      next unless stripe_product

      # Get prices for this product
      prices = Stripe::Price.list({ product: stripe_product.id, active: true })
      next if prices.data.empty?

      # Select appropriate price based on project tier
      price = select_price_by_tier(prices.data, determine_project_tier)
      next unless price

      # Add to invoice
      add_service_to_invoice(invoice_id, stripe_product, price)
    end
  end
  ...
  def find_stripe_product_by_id(product_id)
    return nil unless ENV["STRIPE_API_KEY"].present?

    begin
      # Try to find by product ID first
      Stripe::Product.retrieve(product_id)
    rescue Stripe::InvalidRequestError
      # If not found by ID, search by metadata or name
      products = Stripe::Product.list({ active: true, limit: 100 })

      # Look for product with matching metadata
      product = products.data.find { |p| p.metadata["product_id"] == product_id }

      # If not found, try name matching
      if product.nil?
        name_mapping = {
          "maintenance_retainer" => "maintenance retainer",
          "managed_devops_starter" => "managed devops starter",
          "managed_ecommerce_support" => "managed ecommerce support",
          "managed_api_monitoring" => "managed api monitoring",
          "managed_data_pipeline" => "managed data pipeline",
          "managed_ai_ops" => "managed ai ops",
          "managed_performance_optimization" => "managed performance optimization",
          "managed_security_monitoring" => "managed security monitoring"
        }

        search_name = name_mapping[product_id] || product_id
        product = products.data.find { |p| p.name.downcase.include?(search_name) }
      end

      product
    end
  rescue Stripe::StripeError => e
    Rails.logger.error "Failed to find Stripe product #{product_id}: #{e.message}"
    nil
  end

  def add_legacy_managed_services(invoice_id)
    # Fallback to original logic if pricing service recommendations not available
